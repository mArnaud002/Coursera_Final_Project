...
def get_pie_chart(entered_site):
    ...
    df = filtered_df.groupby('Launch Site')['class'].count().reset_index()
    if entered_site == 'ALL':
        fig = px.pie(df, values='class', 
        names=df['Launch Site'], 
        title='Success Launch according Launch Sites')
        return fig
    else:
...
def scatter_chart(entered_site, select_payload) :
    if entered_site == 'ALL':
        fig = px.scatter(spacex_df, x='Payload Mass (kg)', y='class',
        color="Booster Version Category", 
        title='Payload and Booster Version impact on output results')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
